...
export default function ShoppingListClient({
	...
	const { suggestCategory, rememberCategory: rememberCategoryPreference } =
	...
	const requestClearCompleted = () => {
		setConfirmModal({
			...
			message: `This will remove the ${shoppingList.completedCount} checked item${
				shoppingList.completedCount === 1 ? "" : "s"
			} from your shopping list.`,
			confirmLabel: `Clear Item${shoppingList.completedCount === 1 ? "" : "s"}`,
		});
	};
	...
	const handleConfirmAction = async () => {
		...
		try {
			...
			if (confirmModal.type === "delete-all") {
				succeeded = await shoppingList.handleClearShoppingList();
			}

			if (confirmModal.type === "clear-completed") {
				succeeded = await shoppingList.handleClearCompleted();
			}
			...
			<GroceryPageShell width="shopping" desktopMode="workspace">
				<div className="w-full lg:hidden">
					<MobileShoppingListView
						...
						onRequestClearCompleted={requestClearCompleted}
