...
import { AddtoCartIcon, ArrowRightIcon } from "@/app/components/atoms/icons";
...
const editorCopy = {
	...
	addDescription: "Save something you buy regularly.",
	addSubmitLabel: "Save Quick Add",
	addSavingLabel: "Saving...",

	editTitle: "Edit Quick Add",
	editDescription: "Update this saved grocery item.",
...
export default function DesktopQuickAddView({
	...
	const selectedItem =
		quickAdds.items.find((item) => item.id === selectedItemId) ?? null;
	...
	useEffect(() => {
		if (
			selectedItemId &&
			!quickAdds.items.some((item) => item.id === selectedItemId)
		) {
			setSelectedItemId(null);
		}
	}, [selectedItemId, quickAdds.items]);
	...
	const handleAddToShoppingList = async (item) => {
		...
		try {
			...
			await quickAdds.handleAddToShoppingList(item);
		} finally {
	...
	const sidebarFooter = (
		<>
			<p className="text-sm font-bold">Adding to</p>

			<p className="mt-1 text-sm text-base-content/70">
				{activeList?.title ?? "Shopping List"}
			</p>
	...
	return (
		<div className="grid h-full min-h-0 grid-cols-[17rem_minmax(0,1fr)] overflow-hidden rounded-sm border border-base-300 bg-base-100 xl:grid-cols-[20rem_minmax(0,1fr)]">
			<ItemSidebar
				...
				onUpdateItem={quickAdds.handleUpdateQuickAddItem}
				onDeleteItem={quickAdds.handleRemoveItem}
				onCancelEdit={() => setSelectedItemId(null)}
			...
			<ItemTable
				...
				description={
					<>
						{quickAdds.items.length} saved item
						{quickAdds.items.length === 1 ? "" : "s"}
						{activeList && (
							<>
								{" · "}
								<span className="font-semibold text-primary">
									Adding to {activeList.title}
								</span>
							</>
						)}
					</>
				...
				onChangeQuantity={quickAdds.handleChangeQuantity}
				actionColumn={{
					label: "Add to Cart",
					widthClass: "w-36 min-w-36",
					render: (item) => {
