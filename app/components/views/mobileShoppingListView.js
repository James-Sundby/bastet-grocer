...
export default function MobileShoppingListView({
	...
	const listManager = !isShoppingMode ? (
		<ListManager {...listManagerProps} onSelectList={handleSelectList} />
	) : null;
	...
	return (
		<div className="flex flex-col gap-4">
			<ShoppingListHeader
				...
				onToggleShoppingMode={() => setIsShoppingMode((current) => !current)}
				...
				<div className="mt-auto w-full max-w-xl pt-8">
					{shoppingList.hasCompletedItems ? (
						<div className="grid grid-cols-2 gap-3">
							...
							<DeleteAllButton onDeleteAll={onRequestDeleteAll} />
						</div>
					) : (
						<DeleteAllButton onDeleteAll={onRequestDeleteAll} />
					)}
