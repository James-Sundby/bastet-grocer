...
const editorCopy = {
	addTitle: "Add Item",
	addDescription: "Add something to the current shopping list.",
	addSubmitLabel: "Add Item",
	addSavingLabel: "Adding...",

	editTitle: "Edit Item",
	editDescription: "Update this grocery item.",
...
export default function DesktopShoppingListView({
	...
	}, [selectedItemId, shoppingList.items]);

	const sidebarFooter = (
		<div className="grid gap-2">
			<Link
				href={activeListId ? `/quick-add?list=${activeListId}` : "/quick-add"}
			...
			<div className="flex items-center gap-2">
				{shoppingList.completedCount > 0 && (
					...
					>
						...
						Remove
						{shoppingList.completedCount > 1 &&
							` ${shoppingList.completedCount}`}{" "}
						Checked {shoppingList.completedCount === 1 ? "Item " : "Items "}
					</button>
				)}
	...
	return (
		<div className="grid h-full min-h-0 grid-cols-[17rem_minmax(0,1fr)] overflow-hidden rounded-sm border border-base-300 bg-base-100 xl:grid-cols-[20rem_minmax(0,1fr)]">
			...
			<ItemTable
				title={activeList?.title ?? "Shopping List"}
				description={
					<>
				...
				onChangeQuantity={shoppingList.handleChangeQuantity}
				statusColumn={{
					label: "Status",
					getChecked: (item) => item.completed,
					onChange: shoppingList.handleItemStatusChange,
					getAriaLabel: (item) => `Mark ${item.name} as checked`,
				}}
