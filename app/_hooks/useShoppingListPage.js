...
export function useShoppingListPage({
	...
	const handleChangeQuantity = async (updatedItem, delta) => {
		try {
			const result = await changeItemQuantity(supabase, updatedItem.id, delta);
	...
	const handleClearShoppingList = async () => {
		...
		try {
			...
			notify({
				...
				message: "There was a problem clearing your shopping list.",
				type: "error",
			});
	...
	const handleClearCompleted = async () => {
		...
		try {
			...
			updateItems((prevItems) => prevItems.filter((item) => !item.completed));

			notify({
				title: "Checked items cleared",
				message: `${deletedCount} checked item${
					deletedCount === 1 ? "" : "s"
