...
import { addToast, getFriendlyErrorMessage } from "../components/atoms/toast";
...
export function useQuickAddPage({
	...
	const quickAddQueryKey = userId ?? null;

	const initialQuickAddQueryKey = initialUserId ?? null;
	...
	const [quickAddState, setQuickAddState] = useState(() => ({
		...
		queryKey: hasInitialItems ? quickAddQueryKey : null,
	...
	const errorMessage = hasError ? quickAddState.errorMessage : null;
	...
	const updateQuickAddItems = (updater) => {
		setQuickAddState((currentState) => {
			...
			const nextItems =
				typeof updater === "function" ? updater(currentState.items) : updater;
	...
	useEffect(() => {
		if (!quickAddQueryKey || quickAddState.queryKey === quickAddQueryKey) {
			return undefined;
		}
	...
	}, [supabase, quickAddQueryKey, quickAddState.queryKey]);
	...
	const handleChangeQuantity = async (updatedItem, delta) => {
		try {
			const result = await changeQuickAddQuantity(
				supabase,
				updatedItem.id,
				delta,
			);
	...
	const handleUpdateQuickAddItem = async (itemId, updatedItem) => {
		try {
			...
			const savedItem = await updateQuickAddItem(supabase, itemId, updatedItem);
			...
			const categoryChanged = currentItem?.category !== savedItem.category;
